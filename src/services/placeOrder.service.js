...
const orderConfirmation = async (orderInfo, cart_items, logo, company) => {
  ...
  orderInfo.orderNo = orderInfo.puchase_order_no ? orderInfo.puchase_order_no : '--';
  ...
  readHTMLFile(currentPath + '/resources/views/emails/orderInvoice.html', function (err, html) {
    ...
    var htmlToSend = template({
      ...
      orderNo: orderInfo.orderNo, companyName: orderInfo.companyName, APP_URL: process.env.APP_URL,
      logo: logo
    });
    ...
    const msg = {
      to: 'ganesh.tiwari@adcreators.com.au', // Change to your recipient
...
const placeOrder = async (orderInfo, user) => {
  ...
  */
  ...
  orderInfo.companyName = company['Name'];
  ...
  const currency = orderInfo["Currency Code"];
 
  const order_currency = currency == 'AUD' ? ' ': 'NZD';
  ...
  const addSHQuery = "INSERT INTO [dbo].[" + process.env.BC_DB_NAME + "$Sales Header]" +
    ...
    "VALUES(" +
    "1,'" + incrementedId + "','" + orderInfo["Sell-to Customer No_"] + "','" + orderInfo["Sell-to Customer No_"] + "','" + company["Name"] + "'"
    ...
    + ",'" + bus_posting + "','" + order_currency + "','" + orderInfo['conversion_value'] + "',' ',0"
    + ",' ',' ',' ','" + salesp_code + "',' '"
    + ",2,' ',0,' ',' '"
    + ",0,0,0,' ',' ',' '"
    ...
    + ",'00000000-0000-0000-0000-000000000000',0,0,'" + company["Phone No_"] + "','" + orderInfo["Contact E-mail"] + "'"
    + ",0,' ',0,0,' '"
    + ",1,' ',' ',' ',' '"
    + ",' ',' ',' ',0,0"
  ...
  // let result = await dbConnection.query(addSHQuery);
  //   console.log(result);
  itemQuery = {
    where: {},
    raw: true
  ...
  cart_items.map(item => {
    ...
    queryString =
      ...
      + ",'" + item["item_code"] + "','DKMB','INV-FG','" + formatDate(new Date()) + " 00:00:00','" + item["item_name"].replace("'", "''") + "'"
      + ",' ','Pieces','" + item["quantity"] + "','" + item["quantity"] + "','" + item["quantity"] + "'"
      ...
      + ",0,' ',' ',0,' '"
      + ",' ',' ',' ',' ',0"
      + ",' ',' ','" + bus_posting + "','" + vat_posting + "','" + order_currency + "'"
...
const exchangeOrderItems = async (query) => {
  ...
  await Promise.all(
    ...
    products.map(async (product, key) => {
      ...
      const parent_item = product['DKMTEST16Dec19$Item.Parent Item'] ? product['DKMTEST16Dec19$Item.Parent Item'] : product['No_']
